...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size):
        ...
        self.label_size = 10
        # 编码器层
        self.fc1 = nn.Linear(input_size+self.label_size, 512)
        self.fc2 = nn.Linear(512, latent_size)
        self.fc3 = nn.Linear(512, latent_size)

        # 解码器层
        self.fc4 = nn.Linear(latent_size+self.label_size, 512)
    ...
    def forward(self, x, y):  # 输入图像 x，标签 y 通过编码器和解码器，得到重构图像和潜在变量的均值和对数方差
        x = torch.cat([x, y], dim=1)  # 784 + label
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, labels) in enumerate(train_loader):
            ...
            # TODO进行计算
            predict, mu, log_var = model(data, labels)
        ...
        if (epoch + 1) % sample_interval == 0:
            torch.save(model.state_dict(), f'./out_data/CVAE/vae{epoch + 1}.pth')
            model.eval()
            with torch.no_grad():
                pic_num = 10
                sample_data = torch.randn(pic_num, latent_size).to(device)
                # 生成标签
                sample_labels = torch.arange(10).long().to(device)  # 0-9的标签
                sample_labels_onehot = F.one_hot(sample_labels, num_classes=10).float()
                sample_end = torch.cat([sample_data, sample_labels_onehot], dim=1)  # 连接图片和标签

                sample_img = model.decode(sample_end).cpu()  # 将随机样本输入到解码器中，解码器将其映射为图像
                save_image(sample_img.view(pic_num, 1, 28, 28), f'./out_data/CVAE/sample{epoch}.png', nrow=int(pic_num / 2))
